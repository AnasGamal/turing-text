...
std::make_shared<tcp::socket>(acceptor_.get_executor());
 ...
 boost::bind(&TwilioServer::handle_request,
this, socket, _1));
 }
 void handle_request(std::shared_ptr<tcp::socket> socket, const
...
request_data(boost::asio::buffers_begin(request_buf.data()),

boost::asio::buffers_end(request_buf.data()));
// Find the end of the headers
 size_t header_end = request_data.find("\r\n\r\n");
 if (header_end == std::string::npos)
 {
 std::cerr << "Error: Could not find end of headers" <<
 std::endl;
 return;
 }
// Parse the request parameters
 std::string params_data =
 request_data.substr(request_data.find('?') + 1,
header_end - request_data.find('?') - 1);
 ...
 std::getline(params_stream, param_value, '&'))
 {
...
// Write the JSON object to the console
 ...
 std::cout << "Received request JSON: " << request_json <<
 std::endl;
 std::string response_body =
 request_ptree.get<std::string>("Body.Body");
...
// Send the response
// std::string response_body =
 request_ptree.get<std::string>("Body.ToCountry");
// std::ostringstream response_stream;
// response_stream << "HTTP/1.1 200 OK\r\n";
// response_stream << "Content-Type: text/plain\r\n";
// response_stream << "Content-Length: " <<
...
{
public:
 TwilioClient(const std::string &account_sid, const std::string
&auth_token, const
...
// Set message parameters
 std::string data = "From=" + from_number_ + "&To=" + to_number
 + "&Body=" + message;
// Set POST data
...
void start_twilio()
{
 ...
 std::string message = "Welcome to Turing Text! Do you want to play the
game?";
